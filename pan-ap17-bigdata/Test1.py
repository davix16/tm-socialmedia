# -*- coding: utf-8 -*-
"""
Created on Fri Jun  1 20:39:11 2018
...
for i in range(0, len(docsTraining)):     # len(docsTraining)
    ...
    if(fileName != "truth"):
        ...
        #Hay algun fichero con menos de 100 tweets?? SI -> 7d8903efb5d6a919ba15c49b24f446c8.xml
        #if (len(itemlist) < 100):
...
print("--------------------------------------------")
print('Extraccion palabras BoW. It took {0:0.2f} seconds'.format(time.time() - start_time))


num_features_bow = 10000                                            #ALTERAR
...
#df_female = pd.DataFrame(voca_female)
#df_female.to_csv('bag_of_words_female.txt', index=False, encoding='utf-8-sig')

#df_male = pd.DataFrame(voca_male)
#df_male.to_csv('bag_of_words_male.txt', index=False, encoding='utf-8-sig')
...
for i in range( 0, len(lista)  ):    #Pruebas y verificaciones con 400 (4 docs)
    ...
    if( id_previo != lista[i][0] ):
        ...
        #Guardo datos para report
        report_data_1 += [(id_previo, count_media_palabras, num_emojis/count_media_palabras, num_prep/count_media_palabras, c_masc/count_media_palabras, c_fem/count_media_palabras, sexo)]
...
print("--------------------------------------------")
print('PROCESADO CARACTERISTICAS. It took {0:0.2f} seconds'.format(time.time() - start_time))
...
##GUARDAMOS A TXT LOS DATOS PARA EL REPORT
df = pd.DataFrame(report_data_1)
df.to_csv('report_data_1.txt', index=False)

##GUARDAMOS A TXT LA BOLSA MASCULINA PARA EL REPORT
df = pd.DataFrame(bolsa_masc)
df.to_csv('report_bolsa_masc.txt', index=False)

##GUARDAMOS A TXT LA BOLSA FEMENINA PARA EL REPORT
df = pd.DataFrame(bolsa_fem)
df.to_csv('report_bolsa_fem.txt', index=False)
